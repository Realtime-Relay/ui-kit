...
test.describe('Gauges - Page Load', () => {
  test('page heading renders', async ({ page }) => {
    await expect(page.getByTestId('page-heading')).toHaveText('Gauge Test Page');
  });
...
test.describe('NeedleGauge - SVG Structure', () => {
  test('renders an SVG with role=meter', async ({ page }) => {
    const svg = page.getByTestId('card-needle-default').locator('svg[role="meter"]');
  ...
  test('has pivot circle', async ({ page }) => {
    const circle = page.getByTestId('card-needle-default').locator('svg circle');
...
test.describe('NeedleGauge - Alert Zones', () => {
  ...
  test('zone arcs have opacity=1', async ({ page }) => {
    await page.getByTestId('card-needle-3-zones').scrollIntoViewIfNeeded();
    await page.waitForTimeout(300);
    const zonePaths = page.getByTestId('card-needle-3-zones').locator('svg path[opacity="1"]');
...
test.describe('NeedleGauge - Styling', () => {
  ...
  test('thick arc has larger strokeWidth', async ({ page }) => {
    const bgPath = page.getByTestId('card-needle-thick-arc').locator('svg path').first();
  ...
  test('thin arc has smaller strokeWidth', async ({ page }) => {
    const bgPath = page.getByTestId('card-needle-thin-arc').locator('svg path').first();
    const sw = await bgPath.getAttribute('stroke-width');
    expect(Number(sw)).toBeLessThan(10);
  });

  test('large value font is applied', async ({ page }) => {
    const valueText = page.getByTestId('card-needle-large-value').locator('svg text').nth(2);
  ...
  test('custom min/max style has blue color', async ({ page }) => {
    const minText = page.getByTestId('card-needle-min-max-style').locator('svg text').first();
...
test.describe('NeedleGauge - Zone Values', () => {
  test('zone boundary values 30 and 70 are displayed', async ({ page }) => {
    await page.getByTestId('card-needle-show-zone-values').scrollIntoViewIfNeeded();
...
test.describe('NeedleGauge - Edge Cases', () => {
  test('value=0 renders', async ({ page }) => {
    const svg = page.getByTestId('card-needle-value-zero').locator('svg[role="meter"]');
    await expect(svg).toHaveAttribute('aria-valuenow', '0');
  });

  test('value=max renders', async ({ page }) => {
    const svg = page.getByTestId('card-needle-value-max').locator('svg[role="meter"]');
    await expect(svg).toHaveAttribute('aria-valuenow', '100');
  });

  test('value over max is clamped visually but shows real value', async ({ page }) => {
    await page.getByTestId('card-needle-value-over-max').scrollIntoViewIfNeeded();
  ...
  test('value under min shows real value', async ({ page }) => {
    await page.getByTestId('card-needle-value-under-min').scrollIntoViewIfNeeded();
  ...
  test('loading state shows skeleton', async ({ page }) => {
    ...
    const skeleton = page.getByTestId('card-needle-loading').locator('[style*="shimmer"], [style*="skeleton"]');
    // Should not have a meter SVG
    const svgs = page.getByTestId('card-needle-loading').locator('svg[role="meter"]');
    expect(await svgs.count()).toBe(0);
  });

  test('explicit size renders at specified dimensions', async ({ page }) => {
    await page.getByTestId('card-needle-explicit-size').scrollIntoViewIfNeeded();
...
test.describe('NeedleGauge - Dark Theme', () => {
  test('dark background is applied', async ({ page }) => {
    await page.getByTestId('card-needle-dark-background').scrollIntoViewIfNeeded();
    const container = page.getByTestId('card-needle-dark-background').locator('[style*="background-color"]').first();
...
test.describe('ArcGauge - SVG Structure', () => {
  test('renders an SVG with role=meter', async ({ page }) => {
    await page.getByTestId('arc-section').scrollIntoViewIfNeeded();
    const svg = page.getByTestId('card-arc-default').locator('svg[role="meter"]');
    await expect(svg).toBeVisible();
  });
  ...
  test('zone arcs have opacity=0.25', async ({ page }) => {
    ...
    const zonePaths = page.getByTestId('card-arc-3-zones').locator('svg path[opacity="0.25"]');
    expect(await zonePaths.count()).toBeGreaterThanOrEqual(3);
  });

  test('has a value fill arc (strokeDasharray set)', async ({ page }) => {
    await page.getByTestId('card-arc-default').scrollIntoViewIfNeeded();
    const paths = page.getByTestId('card-arc-default').locator('svg path[stroke-dasharray]');
    expect(await paths.count()).toBeGreaterThanOrEqual(1);
  });
});
...
test.describe('ArcGauge - Variations', () => {
  ...
  test('270° dark has dark background', async ({ page }) => {
    ...
    const container = page.getByTestId('card-arc-270-dark').locator('[style*="background-color"]').first();
    const bg = await container.evaluate(el => getComputedStyle(el).backgroundColor);
    expect(bg).toBe('rgb(15, 23, 42)');
  });

  test('show zone values renders boundary labels', async ({ page }) => {
    await page.getByTestId('card-arc-show-zone-values').scrollIntoViewIfNeeded();
  ...
  test('loading state has no meter SVG', async ({ page }) => {
    ...
    const svgs = page.getByTestId('card-arc-loading').locator('svg[role="meter"]');
    expect(await svgs.count()).toBe(0);
  });
});
...
test.describe('Gauges - Resizable', () => {
  ...
  test('resizable needle gauge SVG fills container', async ({ page }) => {
    await page.getByTestId('resizable-resizable-needle').scrollIntoViewIfNeeded();
