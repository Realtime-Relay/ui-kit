...
test.describe('TimeSeries - Page Load', () => {
  test('page heading renders', async ({ page }) => {
    await expect(page.getByTestId('page-heading')).toHaveText('Time Series Test Page');
  });
...
test.describe('TimeSeries - SVG Structure', () => {
  ...
  test('single device card has a line path', async ({ page }) => {
    const paths = page.getByTestId('card-single-device').locator('path[fill="none"]');
    expect(await paths.count()).toBeGreaterThanOrEqual(1);
  });

  test('three devices card has 3 line paths', async ({ page }) => {
    const paths = page.getByTestId('card-three-devices').locator('path[fill="none"]');
...
test.describe('TimeSeries - Multi-Device Legend', () => {
  test('single device shows metric name only in legend', async ({ page }) => {
    const buttons = page.getByTestId('card-single-device').locator('button[type="button"]');
  ...
  test('three devices shows [device]: metric format', async ({ page }) => {
    const buttons = page.getByTestId('card-three-devices').locator('button[type="button"]');
  ...
  test('formatLegend callback overrides labels', async ({ page }) => {
    const buttons = page.getByTestId('card-format-legend').locator('button[type="button"]');
    const first = await buttons.first().textContent();
    expect(first).toContain('sensor-alpha / value');
  });

  test('multi-metric device shows both metrics', async ({ page }) => {
    const buttons = page.getByTestId('card-multi-metric-single-device').locator('button[type="button"]');
...
test.describe('TimeSeries - Legend Swatch', () => {
  test('swatch is a rounded rectangle (wider than tall)', async ({ page }) => {
    const swatch = page.getByTestId('card-single-device').locator('button[type="button"] span').first();
...
test.describe('TimeSeries - Legend Solo Mode', () => {
  test('clicking a legend item solos it (dims others)', async ({ page }) => {
    ...
    const firstOpacity = await buttons.first().evaluate((el) => getComputedStyle(el).opacity);
    const secondOpacity = await buttons.nth(1).evaluate((el) => getComputedStyle(el).opacity);
...
test.describe('TimeSeries - Line Thickness', () => {
  test('thick lines card has stroke-width=4', async ({ page }) => {
    ...
    const path = page.getByTestId('card-thick-lines').locator('path[fill="none"]').first();
  ...
  test('thin lines card has stroke-width=1', async ({ page }) => {
    ...
    const path = page.getByTestId('card-thin-lines').locator('path[fill="none"]').first();
    const sw = await path.getAttribute('stroke-width');
    expect(sw).toBe('1');
  });
});
...
test.describe('TimeSeries - Point Size', () => {
  ...
  test('points large card has r=5', async ({ page }) => {
    ...
    const circle = page.getByTestId('card-points-large').locator('svg circle').first();
    expect(await circle.getAttribute('r')).toBe('5');
  });

  test('single device card has no circles (no pointSize)', async ({ page }) => {
    const circles = page.getByTestId('card-single-device').locator('svg circle');
    expect(await circles.count()).toBe(0);
  });

  test('per-metric styles card has circles for temp but not humidity', async ({ page }) => {
    await page.getByTestId('card-per-metric-styles').scrollIntoViewIfNeeded();
    const circles = page.getByTestId('card-per-metric-styles').locator('svg circle');
...
test.describe('TimeSeries - Annotations', () => {
  test('point annotations card has dashed vertical lines', async ({ page }) => {
    ...
    const lines = page.getByTestId('card-point-annotations').locator('line[stroke-dasharray="4,3"]');
  ...
  test('range annotations card has shaded rect', async ({ page }) => {
    ...
    const rects = page.getByTestId('card-range-annotations').locator('rect[opacity="0.1"]');
  ...
  test('mixed annotations card has both lines and rects', async ({ page }) => {
    ...
    const lines = page.getByTestId('card-mixed-annotations').locator('line[stroke-dasharray="4,3"]');
    const rects = page.getByTestId('card-mixed-annotations').locator('rect[opacity="0.1"]');
    expect(await lines.count()).toBe(2);
    expect(await rects.count()).toBe(1);
  });
});
...
test.describe('TimeSeries - Start/End & Zoom', () => {
  ...
  test('zoom enabled card has crosshair cursor on overlay', async ({ page }) => {
    await page.getByTestId('card-zoom-enabled').scrollIntoViewIfNeeded();
    const overlay = page.getByTestId('card-zoom-enabled').locator('rect[fill="transparent"]');
    const cursor = await overlay.evaluate((el) => (el as SVGRectElement).style.cursor);
  ...
  test('zoom disabled card has no crosshair cursor', async ({ page }) => {
    ...
    const overlay = page.getByTestId('card-zoom-disabled').locator('rect[fill="transparent"]');
    const cursor = await overlay.evaluate((el) => (el as SVGRectElement).style.cursor);
    expect(cursor).toBe('');
  });

  test('drag-zoom creates a zoom domain and shows reset button', async ({ page }) => {
    await page.getByTestId('card-zoom-enabled').scrollIntoViewIfNeeded();
...
test.describe('TimeSeries - Legend Positions', () => {
  test('legend top card has legend buttons', async ({ page }) => {
    ...
    const buttons = page.getByTestId('card-legend-top').locator('button[type="button"]');
    expect(await buttons.count()).toBe(3);
  });

  test('legend bottom card has legend buttons', async ({ page }) => {
    await page.getByTestId('card-legend-bottom').scrollIntoViewIfNeeded();
    const buttons = page.getByTestId('card-legend-bottom').locator('button[type="button"]');
    expect(await buttons.count()).toBe(3);
  });
  ...
  test('legend right card has legend in row layout', async ({ page }) => {
    ...
    const buttons = page.getByTestId('card-legend-right').locator('button[type="button"]');
    expect(await buttons.count()).toBe(3);
  });

  test('no legend card has no legend buttons', async ({ page }) => {
    await page.getByTestId('card-no-legend').scrollIntoViewIfNeeded();
    const buttons = page.getByTestId('card-no-legend').locator('button[type="button"]');
...
test.describe('TimeSeries - Area & Styles', () => {
  test('area chart has area path with opacity 0.15', async ({ page }) => {
    ...
    const areaPaths = page.getByTestId('card-area-chart').locator('path[opacity="0.15"]');
  ...
  test('alert zones card has zone rects', async ({ page }) => {
    ...
    const zones = page.getByTestId('card-alert-zones').locator('rect[opacity="0.1"]');
  ...
  test('no grid card has no grid lines', async ({ page }) => {
    ...
    const gridLines = page.getByTestId('card-no-grid').locator('line[stroke-dasharray="2,2"]');
...
test.describe('TimeSeries - Tooltip', () => {
  test('hovering shows tooltip', async ({ page }) => {
    ...
    const tooltip = card.locator('div[style*="position: absolute"][style*="pointer-events: none"]');
...
test.describe('TimeSeries - Annotation Mode', () => {
  ...
  test('annotation chart renders with copy cursor when annotation mode ON', async ({ page }) => {
    await page.getByTestId('annotation-mode-section').scrollIntoViewIfNeeded();
  ...
  test('clicking chart in annotation mode fires click event', async ({ page }) => {
    await page.getByTestId('annotation-mode-section').scrollIntoViewIfNeeded();
    const card = page.getByTestId('card-annotation-mode-chart');
    const svg = card.locator('svg');
    const box = await svg.boundingBox();
  ...
  test('zoom is disabled during annotation mode (no zoom on drag)', async ({ page }) => {
    await page.getByTestId('annotation-mode-section').scrollIntoViewIfNeeded();
    const card = page.getByTestId('card-annotation-mode-chart');
    const svg = card.locator('svg');
    const box = await svg.boundingBox();
...
test.describe('TimeSeries - Annotation Hover', () => {
  test('hovering over annotation region shows tooltip with data', async ({ page }) => {
    await page.getByTestId('annotation-mode-section').scrollIntoViewIfNeeded();
  ...
  test('annotation tooltip disappears on mouse leave', async ({ page }) => {
    ...
    if (!svgBox) return;
    await page.mouse.move(svgBox.x + svgBox.width - 20, svgBox.y + svgBox.height / 2);
    await page.waitForTimeout(300);
    expect(await page.getByTestId('annotation-tooltip').count()).toBe(0);
  });

  test('hover log-only chart logs entries without showing tooltip', async ({ page }) => {
    await page.getByTestId('annotation-mode-section').scrollIntoViewIfNeeded();
  ...
  test('hover log shows leave event when mouse exits annotation', async ({ page }) => {
    await page.getByTestId('annotation-mode-section').scrollIntoViewIfNeeded();
    const card = page.getByTestId('card-annotation-hover-log-only');
    await card.scrollIntoViewIfNeeded();
  ...
  test('data point tooltip takes priority over annotation tooltip', async ({ page }) => {
    await page.getByTestId('annotation-mode-section').scrollIntoViewIfNeeded();
    const card = page.getByTestId('card-annotation-hover-with-tooltip');
    await card.scrollIntoViewIfNeeded();
    ...
    const dataTooltip = card.locator('div[style*="pointer-events: none"][style*="z-index: 10"]');
