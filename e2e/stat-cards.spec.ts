...
test.describe("StatCard - Timestamp", () => {
  ...
  test("no timestamp card does NOT show dd MMM yyyy pattern", async ({
    page,
  }) => {
...
test.describe("StatCard - Alert Zones", () => {
  test("green zone (value=20): value text has green color", async ({
    page,
  }) => {
  ...
  test("zone override: value text has purple color, NOT red", async ({
    page,
  }) => {
...
test.describe("StatCard - Styling", () => {
  test("background color applied", async ({ page }) => {
    ...
    await expect(inner.first()).toHaveCSS(
      "background-color",
      "rgb(248, 250, 252)",
    );
  });
...
test.describe("StatCardWithGraph - Sparkline", () => {
  ...
  test('custom graph color: sparkline path has stroke="#ef4444"', async ({
    page,
  }) => {
...
test.describe("StatCardWithGraph - Alert Zone Colors", () => {
  test("red zone: sparkline stroke color matches red zone", async ({
    page,
  }) => {
  ...
  test("zone color override: sparkline stroke stays purple", async ({
    page,
  }) => {
