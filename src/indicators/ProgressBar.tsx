...
export function ProgressBar({
  ...
  const zoneBoundaries =
    hasZones && showZoneValues ? getZoneBoundaries(alertZones, min, max) : [];
          ...
          >
            ...
            {displayZones &&
              alertZones.map((zone, i) => {
                const zoneStart =
                  ((Math.max(zone.min, min) - min) / range) * 100;
            ...
            {showLabel && (
              <span
                style={{
                  ...
                  color:
                    labelFont?.color ??
                    (percentage > 50 ? "#fff" : "currentColor"),
