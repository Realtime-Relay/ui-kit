...
function LivePresencePage({ deviceIdent }: { deviceIdent: string }) {
  ...
  const statusLabel = isLoading ? 'Waiting for data...' : online ? 'Online' : 'Offline';

  return (
    <div style={{ padding: 32, maxWidth: 900, margin: '0 auto' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>Presence Indicator</h1>
      <p style={{ color: '#6b7280', marginBottom: 24 }}>
        All indicators reflect live presence data for <strong>{deviceIdent}</strong>.
      </p>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 20 }}>
        ...
        <Card title="Default (12px)">
          <Row>
            ...
            <span>{deviceIdent} — {statusLabel}</span>
          </Row>
        </Card>
        ...
        <Card title="Small (8px)">
          <Row>
            ...
            <span style={{ fontSize: 13 }}>{deviceIdent} — {statusLabel}</span>
          </Row>
        </Card>
        ...
        <Card title="Extra Large (40px)">
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8, padding: 12 }}>
            ...
            <span style={{ fontSize: 13, color: '#6b7280' }}>{statusLabel}</span>
          </div>
        </Card>
        ...
        <Card title="Custom Colors (Blue / Gray)">
          <Row>
            <PresenceIndicator online={online ?? false} onlineColor="#3b82f6" offlineColor="#9ca3af" size={16} />
        ...
        <Card title="Custom Colors (Purple / Orange)">
          <Row>
            <PresenceIndicator online={online ?? false} onlineColor="#8b5cf6" offlineColor="#f97316" size={16} />
            <span>{statusLabel}</span>
          </Row>
        </Card>
        ...
        <Card title="Custom Colors (Cyan / Pink)">
          <Row>
            <PresenceIndicator online={online ?? false} onlineColor="#06b6d4" offlineColor="#ec4899" size={16} />
            <span>{statusLabel}</span>
          </Row>
        </Card>
        ...
        <Card title="Inline Usage">
          <p style={{ fontSize: 14, lineHeight: 2 }}>
            The device <PresenceIndicator online={online ?? false} size={10} /> <strong>{deviceIdent}</strong> is
        ...
        <Card title="With Event Details" span={2}>
          <Row>
            ...
            <div>
              <div style={{ fontWeight: 600 }}>{deviceIdent} — {statusLabel}</div>
              {lastEvent ? (
                <div style={{ fontSize: 12, color: '#6b7280', marginTop: 4 }}>
                  Last event: <strong>{lastEvent.event}</strong> at{' '}
                  {new Date(lastEvent.data.start).toLocaleString()}
                  {lastEvent.data.stop && (
                    <> — stopped at {new Date(lastEvent.data.stop).toLocaleString()}</>
                  )}
                </div>
              ) : (
        ...
        <Card title="Dashboard Header Style" span={2}>
          <div style={{
            ...
            padding: '12px 16px', backgroundColor: '#f8fafc', borderRadius: 8,
          }}>
            <PresenceIndicator online={online ?? false} size={12} />
            <span style={{ fontFamily: 'monospace', fontSize: 14, fontWeight: 600 }}>{deviceIdent}</span>
            <span style={{
