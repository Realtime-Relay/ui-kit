        ...
        <Card title="Single Device">
          <TimeSeries
            ...
            onHover={(pt) => {
              ...
              if (el && pt)
                el.textContent = `onHover: ${pt.metric} = ${pt.value.toFixed(2)} @ ${new Date(pt.timestamp).toLocaleTimeString()}`;
            }}
            onRelease={(pt) => {
              const el = getHoverDisplay();
              if (el)
...
function AnnotationModeTestSection() {
  ...
  const [events, setEvents] = useState<
    Array<{ id: number; annotationId: number; ts: number; type: string }>
  >([]);
  ...
  const handleAnnotate = (
    ...
    setEvents((prev) =>
      [{ id: nextId.current++, annotationId, ts, type }, ...prev].slice(0, 30),
    );
  ...
  const prebuiltAnnotations: Annotation[] = [
    {
      timestamp: NOW_TS - 30 * 60_000,
      label: "Deploy",
      color: "#3b82f6",
      data: { version: "2.1.0", env: "production", author: "arjun" },
    },
    {
      start: NOW_TS - 45 * 60_000,
      end: NOW_TS - 35 * 60_000,
      label: "Maintenance",
      color: "#8b5cf6",
      data: { ticket: "OPS-412", reason: "DB migration", downtime: "10min" },
    },
  ];
  ...
  return (
    <div style={{ marginBottom: 32 }}>
      ...
      >
        ...
        <div
          data-testid="annotation-event-log"
          style={{
            border: "1px solid #e5e7eb",
            borderRadius: 8,
            padding: 12,
        ...
        >
          ...
          {events.map((e) => (
            <div
              ...
              style={{ borderBottom: "1px solid #f3f4f6", padding: "2px 0" }}
            >
              <span
                data-testid="event-annotation-id"
                style={{ color: "#9ca3af" }}
              >
                #{e.annotationId}
              </span>{" "}
              <span data-testid="event-type" style={{ fontWeight: 600 }}>
                {e.type}
              </span>{" "}
        ...
        <TimeSeries
          ...
          onAnnotationHover={(hover, ann) => {
            ...
            return (
              ...
              >
                <div
                  style={{ fontWeight: 700, marginBottom: 4, color: "#f59e0b" }}
                >
                  {ann.label}
                </div>
      ...
      >
        <Card title="Annotation Hover Log Only" height={300}>
          <TimeSeries
            ...
            onAnnotationHover={(hover, ann) => {
              ...
              setHoverLog((prev) =>
                [`${prefix} ${ann.label}: ${dataStr}`, ...prev].slice(0, 30),
              );
            }}
          />
        </Card>
        ...
        <div
          data-testid="annotation-hover-log"
          style={{
            border: "1px solid #e5e7eb",
            borderRadius: 8,
            padding: 12,
        ...
        >
          ...
          {hoverLog.length === 0 && (
            <div style={{ color: "#9ca3af" }}>
              Hover an annotation on the left chart.
            </div>
          )}
          {hoverLog.map((line, i) => (
            <div
              key={i}
              data-testid="hover-log-entry"
              style={{ borderBottom: "1px solid #f3f4f6", padding: "2px 0" }}
            >
              {line}
            </div>
