  ...
  return (
    <div
      style={{
        ...
        padding: 16,
        ...(resizable
          ? { resize: 'both', overflow: 'auto', minWidth: 300, minHeight: 100, width: '100%', height: height + 60 }
          : {}),
      }}
    >
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: 16, marginBottom: 40 }}>
      <Card
        ...
        description={deviceCount < 3 ? `Next device in 10s…` : 'All devices online'}
...
function LivePage({ deviceIdent, metrics }: { deviceIdent: string; metrics: string[] }) {
  ...
  return (
    <div style={{ padding: 32, maxWidth: 1200, margin: '0 auto' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>State Timeline</h1>
      <p style={{ color: '#6b7280', marginBottom: 16 }}>
        All timelines reflect live <strong>{firstMetric}</strong> data from <strong>{deviceIdent}</strong>.
        The same data is rendered with different state mappers, colors, and styles.
      </p>
      ...
      {/* ── Custom colors ── */}
      <h2 style={{ fontSize: 18, fontWeight: 600, marginBottom: 16 }}>Custom Colors</h2>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: 16, marginBottom: 40 }}>
      ...
      {/* ── State mapper ── */}
      <h2 style={{ fontSize: 18, fontWeight: 600, marginBottom: 16 }}>State Mapper</h2>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: 16, marginBottom: 40 }}>
        <Card
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 40 }}>
        ...
        <Card title="Large labels, custom font" description="Bigger axis labels, monospace font">
          <StateTimeline
            ...
            metricKey={firstMetric}
            styles={{
              label: { fontSize: 14, fontFamily: 'monospace', color: '#1e293b' },
            }}
          />
        </Card>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: 16, marginBottom: 40 }}>
        <Card title="Empty data" description="Shows loading skeleton when data is empty">
          <StateTimeline data={{}} stateMapper={threeStateMapper} metricKey={firstMetric} />
        </Card>

        <Card title="Empty data, loading disabled" description="showLoading=false with empty data — renders nothing">
          <StateTimeline data={{}} stateMapper={threeStateMapper} metricKey={firstMetric} showLoading={false} />
        </Card>

        <Card title="Devices with empty arrays" description="3 devices exist but have no data points" height={160}>
          <StateTimeline
            data={{ 'sensor-alpha': [], 'sensor-bravo': [], 'sensor-charlie': [] }}
            stateMapper={threeStateMapper}
            metricKey={firstMetric}
          />
        </Card>

        <Card title="Mix of empty and populated" description="One device has data, two have empty arrays" height={160}>
          <StateTimeline
            data={{ [deviceIdent]: data, 'sensor-offline-1': [], 'sensor-offline-2': [] }}
            stateMapper={threeStateMapper}
            metricKey={firstMetric}
          />
        </Card>
      ...
      {/* ── Stacked ── */}
      <h2 style={{ fontSize: 18, fontWeight: 600, marginBottom: 16 }}>Stacked (Multi-Device)</h2>
      ...
      {(() => {
        return (
          <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: 16, marginBottom: 40 }}>
            ...
            <Card title="3 Devices — Custom Temperature Colors" description="Temperature palette across devices" height={160}>
              <StateTimeline
                data={stackedData}
                stateMapper={temperatureMapper}
                metricKey={firstMetric}
            ...
            <Card title="2 Devices — Online / Offline" description="Primary vs backup device" height={120}>
              <StateTimeline
                data={twoDevices}
                stateMapper={onlineOfflineMapper}
                metricKey={firstMetric}
            ...
            <Card title="Tall Rows (rowHeight=48)" description="Larger row height for better readability with many states" height={240}>
              <StateTimeline
                data={stackedData}
            ...
            <Card title="Resizable Stacked" description="Drag the corner to resize and observe responsive behavior" height={160} resizable>
              <StateTimeline
                data={stackedData}
                stateMapper={threeStateMapper}
                metricKey={firstMetric}
            ...
            <Card title="Dark Theme Stacked" description="3 devices with dark background" height={180}>
              <StateTimeline
                data={stackedData}
                stateMapper={collapsedMapper}
                metricKey={firstMetric}
            ...
            <Card title="Long Device Names (auto-sized labels)" description="Label column auto-measures to fit long names" height={160}>
              <StateTimeline
            ...
            <Card title="Labels on Right" description="labelAlign='right' places device names after the bars" height={160}>
              <StateTimeline
                data={stackedData}
                stateMapper={temperatureMapper}
                metricKey={firstMetric}
            ...
            <Card title="Long Names + Right Aligned" description="Auto-sized labels on the right side" height={160}>
              <StateTimeline
                data={longNameDevices}
                stateMapper={fiveStateMapper}
                metricKey={firstMetric}
      ...
      {/* ── Dynamic devices ── */}
      <h2 style={{ fontSize: 18, fontWeight: 600, marginBottom: 16 }}>Dynamic Devices</h2>
      <p style={{ color: '#6b7280', marginBottom: 16, fontSize: 13 }}>
        New devices appear at 10-second intervals. Watch the timeline expand as devices come online.
      </p>
      <DynamicDevicesDemo data={data} deviceIdent={deviceIdent} firstMetric={firstMetric} />

      {/* ── Error callback ── */}
      <h2 style={{ fontSize: 18, fontWeight: 600, marginBottom: 16 }}>Error Handling</h2>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: 16, marginBottom: 40 }}>
        <Card title="onError callback (check console)" description="Data includes an invalid timestamp — onError fires">
          <StateTimeline
            data={{ [deviceIdent]: [{ timestamp: NaN, [firstMetric]: 50 }, ...data.slice(0, 30)] }}
            stateMapper={threeStateMapper}
            metricKey={firstMetric}
