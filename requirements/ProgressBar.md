...
| Prop | Type | Required | Default | Description |
|---|---|---|---|---|
| `lastUpdated` | `Date \| number` | No | -- | Timestamp of the last data update. Only displayed when `showLastUpdated` is true. |
| `showLastUpdated` | `boolean` | No | `false` | Whether to show the formatted timestamp below the bar. |
...
When `showLastUpdated = true` and `lastUpdated` is not null/undefined, a `<div>` is rendered below the bar container:
...
Content: `formatTimestamp(lastUpdated)`.
